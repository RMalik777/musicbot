...
class Music(commands.Cog):
    ...
    async def ensure_voice_state(self, ctx: commands.Context):
        if not ctx.author.voice or not ctx.author.voice.channel:
            embedVar = discord.Embed(description="You are not connected to any voice channel.", color=discord.Color.red())
            await ctx.send(embed=embedVar)

        if ctx.voice_client:
            if ctx.voice_client.channel != ctx.author.voice.channel:
                embedVar = discord.Embed(description="Bot is already in a voice channel.", color=discord.Color.red())
                await ctx.send(embed=embedVar)



bot = commands.Bot(command_prefix='-', case_insensitive=True, description="Karena Groovy sudah tidak ada. Sekarang saya yang menggantikan")
